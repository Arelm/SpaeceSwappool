...
const { SpaceswapPriceFeed } = require("./SpaceswapPriceFeed");
...
async function createPriceFeed(logger, web3, networker, getTime, config) {
    const Uniswap = getTruffleContract("Spaceswap", web3, "latest");
    const BEP20 = getTruffleContract("ExpandedERC20", web3, "latest");
    ...
    if (config.type === "cryptowatch") {
    ...
    } else if (config.type === "Spaceswap") {
        const requiredFields = ["SpaceswapAddress", "twapLength", "lookback"];
        ...
        return new SpaceswapPriceFeed(
            logger,
            Spaceswap.abi,
            BEP20.abi,
            web3,
            config.SpaceswapAddress,
            config.twapLength,
            config.lookback,
            getTime,
            config.invertPrice, // Not checked in config because this parameter just defaults to false.
...
async function getSpaceswapPairDetails(web3, syntheticTokenAddress, collateralCurrencyAddress) {
    const networkId = await web3.eth.net.getId();

    if (process.env.SpaceSWAP_ADDRESS) {
        // Used for mock uniswap pair contracts.
        return { address: process.env.SpaceSWAP_ADDRESS, inverted: false };
...
async function createSpaceswapPriceFeedForFinancialContract(
    logger,
    web3,
    networker,
    getTime,
    ...
    if (!financialContractAddress) {
        throw new Error("createSpaceswapPriceFeedForFinancialContract: Must pass in an `financialContractAddress`");
    }
    ...
    const { pairAddress, inverted } = await getSpaceswapPairDetails(web3, syntheticTokenAddress, collateralCurrencyAddress);

    if (!pairAddress && !config) {
        throw new Error(
            "No Spaceswap Pair address found and no override config provided. Either set SpaceSWAP_ADDRESS, use a network where there is an official Uniswap V2 deployment or set a default `config` value"
        );
    }
    ...
    logger.debug({
        at: "createSpaceswapPriceFeedForFinancialContract",
